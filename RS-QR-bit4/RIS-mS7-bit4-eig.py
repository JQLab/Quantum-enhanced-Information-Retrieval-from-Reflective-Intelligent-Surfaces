...
# ---------------------------
def build_RIS_NPM_symbols(NI, NO):
    psi_inner = np.array([0, np.pi/2, np.pi, 3*np.pi/2])
    alpha_inner = np.sqrt(NI) * np.exp(1j * psi_inner)
    psi0 = np.pi/4 - np.pi/12
    psi_outer = psi0 + 2*np.pi*np.arange(12)/12
...
def EIG_TRM_for_beta(alpha_list, prior, rr, beta, tau, n_grid=400):
    """
    Compute TRM EIG for first-click SPD on interval [0, tau] using per-hypothesis KL form:
        EIG = Σ_k π_k [ ∫_0^τ p_k(t) log(p_k(t)/p_mix(t)) dt + S_k log(S_k/S_mix) ]
    Also returns expected time E[T].
    """
    eps = 1e-12
    M = len(alpha_list)
    rates = np.array([rate_per_unit_time(alpha_list[k], rr, beta)
                      for k in range(M)])

    # Grid for t ∈ [0, τ)
    tgrid = np.linspace(0.0, tau, n_grid+1)[:-1] + 0.5*(tau / n_grid)
    dtg = tau / n_grid

    # First-click density p_k(t) = λ_k e^{-λ_k t}
    pk_t = np.zeros((M, n_grid))
    for k in range(M):
        lam = rates[k]
        pk_t[k, :] = lam * np.exp(-lam * tgrid)
    pmix_t = np.dot(prior, pk_t)
    pmix_t = np.clip(pmix_t, eps, None)

    # ----- EIG (first-click part) -----
    EIG_click = 0.0
    for k in range(M):
        valid = pk_t[k, :] > 0
        Dk = np.sum(pk_t[k, valid] * np.log(pk_t[k, valid] / pmix_t[valid])) * dtg
        EIG_click += prior[k] * Dk

    # ----- Add survival term -----
    Sk = np.exp(-rates * tau)
    Sk = np.clip(Sk, eps, None)
    Smix = np.dot(prior, Sk)
    Smix = max(Smix, eps)
    survival_term = np.sum(prior * Sk * np.log(Sk / Smix))
    EIG_total = EIG_click + survival_term
    #print('EIG',EIG_click, survival_term)
    return EIG_total
...
def adaptive_hybrid_multiarea_receiver(
    ...
    while elapsed < T_full - 1e-12:
        ...
        # # Replace any value smaller than 2e-6 with 2e-6
        # wait_t_list = [max(t, 2e-6) for t in wait_t_list]
        ...
        # if min(wait_t_list)<1e-6:
        #     print(wait_t_list,'wait--t',xtx)
...
if __name__ == "__main__":
    ...
    numS  = 7
    n0 = 1/numS
    area_efficiency = [0.99, 0.69, 0.69, 0.69, 0.69, 0.69, 0.69]
    # [0.99,0.89]#[0.60]#[0.57, 0.55, 0.60]#[0.60 for _ in range(numS)]#[0.59, 0.58, 0.60, 0.57]#


    num_trials = 2*10000#num_trials = 4*10000

    Tf_list = [13]#[10**i for i in range(-1,6)]
    ...
    for tfu in Tf_list:
        n = 1000
        T_full = tfu  # 100
        NI, NO =20.0*n, 80.0*n  # 20.0, 80.0   # example ring energies (tune as you like)
        alphas00 = build_RIS_NPM_symbols(NI, NO)
        ...
        Tsum_avg = np.mean(Tsum_list)
        Pe_avg_list.append(Pe_avg)
        T_avg_list.append(T_avg)
        Tsum_avg_list.append(Tsum_avg)
        ...
        print(f"Tfu={tfu}: Average Pe={Pe_avg:.4e}, Avg Time={T_avg}")
    ...
    overall_save = {
        ...
        'Tsum_avg_list': np.array(Tsum_avg_list),
    }
